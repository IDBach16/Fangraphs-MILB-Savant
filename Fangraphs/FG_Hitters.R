...
test <- jsonlite::fromJSON("https://www.fangraphs.com/api/leaders/major-league/data?age=&pos=all&stats=bat&lg=all&qual=0&season=2024&season1=2024&startdate=2024-03-01&enddate=2024-11-01&month=0&hand=&team=0&pageitems=10000&pagenum=1&ind=0&rost=0&players=&type=8&postseason=&sortdir=default&sortstat=WAR")
...
db3 = dbConnect(SQLite(),"FG_Hitters.sqlite")
dbWriteTable(db3,"FG_Hitters",FG,overwrite=T)
dbDisconnect(db3)

# Write the first 100 rows to a CSV file
write.csv(FG[1:50, ], "Hitter_Database_first_50.csv", row.names = FALSE)
